...
void pid_init1(pid_struct_t *pid,   fp32 PID[3], fp32 max_out, fp32 max_iout)   //pid?????'??????????????????????????????????????
{
    if (pid == NULL || PID == NULL)
    {
        return;
...
fp32 pid_calc_a(pid_struct_t *pid, fp32 fdb, fp32 set)   //????????????????????????????????   
{
    pid->error[1] = pid->error[0];
    pid->set = set;
    pid->fdb = fdb;
...
pid_struct_t motor_pid_chassis[4];//??????PID
