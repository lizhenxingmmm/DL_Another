...
void Pitch_pc_send();
void Pitch_pc_get(uint8_t rx_buffer[]);
...
void Pitch_pc_send()
{
	uint8_t Temp_pitch_pc[3];
	Temp_pitch_pc[0] = 0x02;
	Temp_pitch_pc[1] = ((uint16_t)Err_pitch >> 8) & 0xff;
	Temp_pitch_pc[2] = (uint16_t)Err_pitch & 0xff;
	HAL_UART_Transmit_DMA(&huart1,Temp_pitch_pc,3);
}

void Pitch_pc_get(uint8_t rx_buffer[])
{
	Remember_pitch = (uint16_t)(rx_buffer[1] << 8 | rx_buffer[2]);
}